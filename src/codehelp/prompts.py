...
def make_main_prompt(code: str, error: str, issue: str, context: str | None = None) -> str:
    error = error.rstrip()
    issue = issue.rstrip()
    ...
    # ]
    prom = common_template_user.render(code=code, error=error, issue=issue)
    # ✅ Print what is being sent to the LLM for manual check
    print("\n🟡 Prompt sent to LLM:\n")
    print(prom)
    print("\n🔍 Included Fields: ", end="")
    print(f"{'code' if code else ''} {'error' if error else ''} {'issue' if issue else ''}\n")
    # return common_template_user.render(code=code, error=error, issue=issue)
    return prom
...
def make_sufficient_prompt(code: str, error: str, issue: str, context: str | None) -> list[ChatCompletionMessageParam]:
    ...
    # Optimization: if error and issue are identical, discard issue
    # if error == issue:
    #     issue =  "Can you help me understand what this error means and how to fix it?"

    sys_job = "to evaluate whether a student's query contains sufficient detail for you to provide assistance"
    
    # Generate the system and user prompt
    system_prompt = common_template_sys1.render(job=sys_job, code=code, error=error, issue=issue, context=context)
    user_prompt = common_template_user.render(code=code, error=error, issue=issue)

    # Print the generated system and user prompts
    print("Generated System Prompt for Sufficient Check:")
    print(system_prompt)
    print("Generated User Prompt for Sufficient Check:")
    print(user_prompt)
...
# """
def make_cleanup_prompt(response_text: str) -> str:
    # Print the original response text
    print("Original Response Text:")
    print(response_text)

    cleaned_response = f"""The following was written to help a student in a CS class.  However, any example code (such as in ``` Markdown delimiters) can give the student an assignment's answer rather than help them figure it out themselves.  We need to provide help without including example code.  To do this, rewrite the following to remove any code blocks so that the response explains what the student should do but does not provide solution code.
---
{response_text}
---
Rewritten:
...
"""},
    ...
    # Print the generated set of messages
    print("Generated Topic-Specific Prompts:")
    for msg in messages:
        print(msg)
...
def make_chat_sys_prompt(topic: str, context: str) -> str:
    # Generate the prompt
    prompt = chat_template_sys.render(topic=topic, context=context)
