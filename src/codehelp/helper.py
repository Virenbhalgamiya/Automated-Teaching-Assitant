...
def load_models():
    xgb_model = joblib.load("G:\\Codehelp\\src\\codehelp\\error_question_classifier_xgb_with_3_dataset.pkl")
    model = SentenceTransformer("G:\\Codehelp\\src\\codehelp\\sbert_model_dir", device='cpu')
# or load from pickle if custom
    return xgb_model, model

xgb_model , model = load_models() # added by viren

def is_related(error, issue):
    error_vec = model.encode(error,convert_to_numpy=True)
    issue_vec = model.encode(issue,convert_to_numpy=True)
...
def help_form(llm: LLMConfig, query_id: int | None = None, class_id: int | None = None, ctx_name: str | None = None) -> str | Response:
    db = get_db()
    auth = get_auth()
    print("help_form from helper.py is called")
...
def help_view(query_id: int) -> str | Response:
    print("help view method from helper.py is called.....")
...
async def run_query_prompts(llm: LLMConfig, context: ContextConfig | None, code: str, error: str, issue: str) -> tuple[list[dict[str, str]], dict[str, str]]:
    ...
    task_main = asyncio.create_task(
        get_completion(
            client,
            model=model,
            prompt=prompts.make_main_prompt(code, error, issue, context_str),
        )
    )
    task_sufficient = asyncio.create_task(
        get_completion(
            client,
            model=model,
            prompt=prompts.make_sufficient_prompt(code, error, issue, context_str),
        )
    )
...
def run_query(llm: LLMConfig, context: ContextConfig | None, code: str, error: str, issue: str) -> int:
    print("run query from helper.py is called....")
...
def record_query(context: ContextConfig | None, code: str, error: str, issue: str) -> int:
    ...
    print("record query method from helper.py method is called....")
...
def record_response(query_id: int, responses: list[dict[str, str]], texts: dict[str, str]) -> None:
    db = get_db()
    print("record response method from helper.py is called....")
    db.execute(
...
def help_request(llm: LLMConfig) -> Response:
    auth = get_auth()
    print("help_request is callled................... 1" )
    ...
    if not is_related(error, issue):
        print("Inputs are unrelated. Showing message on response page.")
        # Create a fake query_row-like dict for rendering help_view.html
        from datetime import datetime

        query_fake = SimpleNamespace(
            code=code,
            error=error,
            issue=issue,
            query_time=datetime.utcnow(),  # dummy time to avoid localtime error
            context_name=None,
            display_name="",
        ...
        history = get_history()
        return render_template("help_view.html", query=query_fake, responses={"error": "⚠️ The inputs don't seem related. Please re-enter."}, history=history, topics=[])
...
def load_test(llm: LLMConfig) -> Response:
    ...
    if auth['display_name'] != 'load_test':
        return abort(403)
    print("load test method from helper.py is called........")
...
def post_helpful() -> str:
    db = get_db()
    auth = get_auth()
    print("Post helpful method from helper.py is called...")
...
def get_topics_html(llm: LLMConfig, query_id: int) -> str:
    ...
    print("get topics html from helper.py method is called ")
    if not topics:
...
def get_topics_raw(llm: LLMConfig, query_id: int) -> list[str]:
    topics = get_topics(llm, query_id)
    print("Get topics raw method is called.....")
    return topics
...
def get_topics(llm: LLMConfig, query_id: int) -> list[str]:
    ...
    print("Get topics method from helper.py is called..")
